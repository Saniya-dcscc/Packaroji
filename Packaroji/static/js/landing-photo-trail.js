 ...
 * Scoped to the single home-page landing photo; no external libraries.
 */
(function () {
  'use strict';
  ...
  function init() {
    ...
    // Bottom layer: the existing photo, only this layer gets the richer grade.
    baseImg.classList.add('landing-trail-base');
    baseImg.style.filter = 'saturate(1.55) hue-rotate(-6deg) brightness(1.08) contrast(1.03)';

    // Required duplicate image: same source, same footprint, directly over the base.
    // The visible reveal is rendered into the canvas immediately above it.
    ...
    function noise(seed, t) {
      return (
        Math.sin(seed * 1.71 + t * 1.23) * 0.32 +
    ...
    function drawFront() {
      ...
      if (points.length) {
        ...
        for (var i = 0; i < points.length; i += 1) {
          if (points[i].radius > 0.5 && points[i].alpha > 0.01) {
            drawBlob(ctx, points[i]);
            ctx.fill();
          }
        }
      }
    ...
    function update() {
      headRadius = lerp(headRadius, hovering ? 140 : 0, hovering ? 0.14 : 0.04);
    ...
    if (baseImg.complete && baseImg.naturalWidth) {
      ...
      // Keep the effect dormant until the existing image finishes loading.
      baseImg.addEventListener('load', function () { ensureRunning(); }, { once: true });
    }
  }
