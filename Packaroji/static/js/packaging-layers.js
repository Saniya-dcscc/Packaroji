(function () {
    ...
    function initPackagingLayerVideo() {
        ...
        function onWheel(event) {
            ...
            if (direction > 0) {
                if (step >= 3) { finishStoryForward(); return; }
                event.preventDefault();
                event.stopImmediatePropagation();
                completed = false;
                setFilmVisible(true);
            ...
            if (step < 0) return;
            if (step === 0) { resetBeforeLeavingBackwards(); return; }
            event.preventDefault();
            event.stopImmediatePropagation();
            completed = false;
            setFilmVisible(true);
        ...
        film.addEventListener("playing", function () { film.pause(); });
        film.pause();
        film.load();
        film.pause();
        film.addEventListener("loadedmetadata", function () { film.pause(); setTime(0); });
