document.addEventListener("DOMContentLoaded", () => {
    ...
    /* =========================================================
       HERO MASCOT — EYES FOLLOW CURSOR (v26)
    ========================================================== */
    ...
    (() => {
        const initMascotEyes = () => {
            ...
            const move = (event) => {
                if (reducedMotion.matches) { resetEyes(); return; }
                const rect = svg.getBoundingClientRect();
                if (!rect.width || !rect.height) return;

                const mx = (event.clientX - rect.left) * (260 / rect.width);
                const my = (event.clientY - rect.top) * (260 / rect.height);

                eyes.forEach((eye) => {
                    const dx = mx - eye.cx;
                    const dy = my - eye.cy;
                    const angle = Math.atan2(dy, dx);
                    const distance = Math.min(7.5, Math.hypot(dx, dy) / 10);
                    eye.el.setAttribute("cx", (eye.cx + Math.cos(angle) * distance).toFixed(2));
                    eye.el.setAttribute("cy", (eye.cy + Math.sin(angle) * distance).toFixed(2));
                });
            };
