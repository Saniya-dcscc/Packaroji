document.addEventListener("DOMContentLoaded", () => {
    ...
    /* =========================================================
       PRODUCT CAROUSEL — FINAL STRAIGHT ROW
       Fixed-size cards + flex track. No absolute-positioned slides.
    ========================================================== */
    (() => {
        ...
        sliders.forEach((slider) => {
            ...
            function measure() {
                ...
                if (w >= 1200) {
                    cardWidth = clamp(w * 0.145, 205, 235);
                    gap = 28;
                } else if (w >= 900) {
                    cardWidth = clamp(w * 0.19, 195, 225);
                    gap = 24;
                } else if (w >= 600) {
                    cardWidth = clamp(w * 0.27, 180, 210);
                    gap = 20;
                } else {
                    cardWidth = clamp(w * 0.62, 165, 195);
                    gap = 16;
                }
                step = cardWidth + gap;
            ...
            function render() {
                // Start at the first original copy (copy 0), with a small left inset.
                const leftInset = Math.max(18, (viewport.clientWidth - Math.min(viewport.clientWidth, count * step)) / 2);
                const base = -2 * count * step + leftInset;
            ...
            function desiredSpeed() {
                ...
                // Gentle cursor steering; center is almost stationary, edges are still slow.
                return pointerBias * 18;
            }
