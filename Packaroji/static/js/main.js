document.addEventListener("DOMContentLoaded", () => {
    ...
    /* =========================================================
       FOOTER MASCOT — EYES FOLLOW CURSOR
    ========================================================== */

    (() => {
        const svg = document.getElementById("footerMascot");
        const leftPupil = document.getElementById("footerLeftPupil");
        const rightPupil = document.getElementById("footerRightPupil");
        ...
        const resetEyes = () => {
            eyes.forEach((eye) => {
                eye.el.setAttribute("cx", eye.cx);
                eye.el.setAttribute("cy", eye.cy);
            });
        };
        ...
        const move = (event) => {
            ...
            eyes.forEach((eye) => {
                const dx = mx - eye.cx;
                const dy = my - eye.cy;
                const dist = Math.min(6, Math.hypot(dx, dy) / 6);
                const angle = Math.atan2(dy, dx);

                eye.el.setAttribute(
                    "cx",
                    eye.cx + Math.cos(angle) * dist
                );
                eye.el.setAttribute(
                    "cy",
                    eye.cy + Math.sin(angle) * dist
                );
            });
        };
        ...
        const handlePrefChange = () => {
            if (reducedMotion.matches || mobile.matches) resetEyes();
        };

        reducedMotion.addEventListener("change", handlePrefChange);
        mobile.addEventListener("change", handlePrefChange);
    })();
