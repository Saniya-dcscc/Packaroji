document.addEventListener("DOMContentLoaded", () => {
    ...
    /* =========================================================
       PRODUCT CAROUSEL — RECORDING-STYLE ROW
       Individual cards, continuous slow movement, cursor steering,
       drag anywhere, and touch swipe.
    ========================================================== */
    (() => {
        ...
        sliders.forEach((slider) => {
            ...
            // Five copies give enough content on both sides without changing the cards themselves.
            ...
            function measure() {
                const first = track.querySelector('[data-product-slide]');
                if (first) {
                    const rect = first.getBoundingClientRect();
                    if (rect.width > 0) cardWidth = rect.width;
                }
                const style = getComputedStyle(track);
                const g = parseFloat(style.columnGap || style.gap || '24');
            ...
            function render() {
                // Center the middle copy so several complete cards are visible immediately.
                const start = -(2 * cycle) + Math.max(18, (viewport.clientWidth - Math.min(viewport.clientWidth, count * step + (count - 1) * gap)) / 2);
                track.style.transform = `translate3d(${start + offset}px,0,0)`;
            }

            function autoSpeed() {
                if (window.innerWidth <= 650) return 7;
                if (window.innerWidth <= 1000) return 8.5;
                return 10;
            }

            function desiredSpeed() {
                if (!pointerInside || dragging) return autoSpeed();
                // Edges steer slowly; center keeps the normal slow auto-scroll.
                const steering = pointerBias * 14;
                return autoSpeed() + steering;
            }
