document.addEventListener("DOMContentLoaded", () => {
    ...
    /* =========================================================
       ...
       Desktop only. The cursor is drawn into a tiny PNG data URL
       so normal CSS cursor rules can use it. It automatically
       returns to the normal cursor after ~1.8s, or on mouse move.
    ========================================================== */

    const packarojiEmojiCursor = (() => {
        const finePointer = window.matchMedia && window.matchMedia("(pointer: fine)").matches;
        if (!finePointer) return { show() {} };
        ...
        function show(type) {
            if (!document.body || !cursors[type]) return;
            active = true;
            if (timer) clearTimeout(timer);
            document.body.style.cursor = `url("${cursors[type]}"), auto`;
            timer = setTimeout(reset, 1800);
        }

        // The first real mouse movement after the action restores the default cursor.
        document.addEventListener("mousemove", reset, { passive: true });

        // Hook: any explicitly marked back/shop link gets the 🥹 cursor.
        document.addEventListener("click", event => {
            const back = event.target.closest?.("[data-packaroji-cursor='sad']");
            if (back) show("sad");
        });

        // Hook: add-to-cart code calls this after a successful server response.
        window.packarojiHappyCursor = () => show("happy");

        // Hook: successful checkout redirects here with ?placed=1.
        try {
            ...
            button.addEventListener(
                ...
                async event => {
                    ...
                    try {
                        ...
                        if (window.packarojiHappyCursor) {
                            window.packarojiHappyCursor();
                        }
