        ...
        sliders.forEach((slider) => {
            ...
            const animate = (now) => {
                ...
                if (!dragging) {
                    ...
                    // While the cursor is over the slider, horizontal cursor
                    // movement controls the carousel direction. When the
                    // cursor leaves, the existing automatic right-to-left
                    // motion resumes unchanged.
                    if (hover && cursorX !== null) {
                        // Cursor movement is handled by mousemove below.
                    } else {
                        let s = speed();
            ...
            slider.addEventListener(
                ...
                (event) => {
                    ...
                    const dx = event.clientX - cursorX;
                    cursorX = event.clientX;

                    if (!dx || !spacing) return;

                    // Right cursor movement = right; left cursor movement = left.
                    phase += (dx / spacing) * 0.95;
                    wrapPhase();
                    render();
                },
                { passive: true }
