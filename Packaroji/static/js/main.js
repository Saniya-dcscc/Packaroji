        ...
        filterButtons.forEach(button => {
            button.addEventListener("click", () => {
                ...
                productCards.forEach(card => {
                    ...
                    const show = normalized === "all" || category === normalized;
                    card.style.display = show ? "" : "none";
                    if (show) visible += 1;
                });
                if (filterNote) filterNote.textContent = normalized === "all" ? "Showing all packaging options" : visible ? `Showing ${filter} options` : `More ${filter.toLowerCase()} options can be added here as the range grows.`;
