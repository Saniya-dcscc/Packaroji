...
PRODUCT_CATEGORIES = [
    ...
    {"name": "Bakery Packaging", "icon": "🧁", "image": "catalogue/window-bakery-boxes.jpg", "description": "Bakery boxes, cake boxes and stand-up pouches"},
...
PRODUCTS = [
    ...
    # Layer 2 — Bakery packaging
    {
        ...
        "category": "Bakery Packaging", "layer": "Layer 2 — Main Portion Packaging", "brandable": True,
...
def product_collection_page(collection_slug):
    collections = {
        "food-wrapping": {
            "name": "Food Wrapping",
            "description": "Layer 1 packaging that touches the food directly: foils, kraft & butter paper rolls and wrap sheets.",
            "products": lambda: [p for p in PRODUCTS if p["category"] == "Food Wrapping"],
        },
        "main-portion-packaging": {
            "name": "Main Portion Packaging",
            "description": "Layer 2 packaging across aluminium foil, kraft paper, bakery packaging and sugarcane bagasse.",
            "products": lambda: [p for p in PRODUCTS if p["layer"].startswith("Layer 2")],
        },
        "takeout-delivery": {
            "name": "Carry Bags & Pouches",
            "description": "Takeout and delivery packaging, including kraft carry bags plus the catalogue's bakery-special stand-up pouches and bags.",
            "products": lambda: [p for p in PRODUCTS if p["category"] == "Takeout & Delivery" or p["slug"] == "stand-up-pouches-bags"],
        },
        "branding": {
            "name": "Branding Options",
            "description": "Browse the full catalogue through Packaroji's branding/custom-printing collection. Every catalogue product is marked as brandable so this page shows the complete branded range.",
