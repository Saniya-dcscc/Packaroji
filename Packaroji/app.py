...
def product_collection_page(collection_slug):
    collections = {
        "food-wrapping": {
            "name": "Layer 1 — Food Wrapping",
            "description": "The layer that touches the food directly: aluminium foil, kraft & butter paper rolls, wrap sheets and the catalogue's branding option.",
            "products": lambda: [p for p in PRODUCTS if p.get("layer") == "Layer 1 — Food Wrapping"],
        },
        "main-portion-packaging": {
            "name": "Layer 2 — Main Portion Packaging",
            "description": "Choose one of the three Layer 2 material groups from the catalogue: Aluminium Packaging, Kraft Paper Packaging or Sugarcane Bagasse.",
            "products": lambda: [p for p in PRODUCTS if p.get("layer") == "Layer 2 — Main Portion Packaging"],
        },
        "takeout-delivery": {
            "name": "Layer 3 — Takeout & Delivery",
            "description": "The Layer 3 takeout and delivery range from the catalogue: SOS kraft bags and wide-base die-cut bags in the listed GSM options.",
            "products": lambda: [p for p in PRODUCTS if p.get("layer") == "Layer 3 — Takeout & Delivery"],
        },
        "branding": {
            "name": "Branding Options — Full Catalogue",
            "description": "All catalogue products in one place for branding and custom-printing enquiries. Products marked as brandable are shown here.",
            "products": lambda: [p for p in PRODUCTS if p.get("brandable")],
        },
    }
    collection = collections.get(collection_slug.strip().lower())
    if not collection:
        return render_template("404.html"), 404
    ...
    layer2_materials = []
    selected_material = ""
    if collection_slug.strip().lower() == "main-portion-packaging":
        layer2_materials = ["Aluminium Foil Packaging", "Kraft Paper Packaging", "Sugarcane Bagasse"]
        selected_material = request.args.get("material", "").strip()
        if selected_material not in layer2_materials:
            selected_material = layer2_materials[0]
        products = [p for p in products if p.get("category") == selected_material]
    ...
    return render_template(
        ...
        category_icon="✨" if collection_slug == "branding" else "📦",
        is_all_products=False,
        is_collection=True,
