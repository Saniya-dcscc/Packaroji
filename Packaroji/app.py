...
REVIEW_UPLOAD_DIR = os.path.join(
    BASE_DIR,
    "uploads",
    "reviews"
)
...
# Where Packaroji receives new-order alerts. Override on Render if needed.
...
PRODUCT_CATEGORIES = [
    {"name": "Bowls", "icon": "🥣", "description": "Meal, salad & takeaway bowls"},
    {"name": "Cups", "icon": "🥤", "description": "Drink, sauce & portion cups"},
    {"name": "Plates", "icon": "🍽️", "description": "Plates for food service"},
    {"name": "Bags", "icon": "🛍️", "description": "Carry bags for takeaway"},
    {"name": "Containers", "icon": "🥡", "description": "Food containers for orders"},
    {"name": "Boxes", "icon": "📦", "description": "Meal, bakery & display boxes"},
    {"name": "Trays", "icon": "🧺", "description": "Serving & catering trays"},
    {"name": "Pouches", "icon": "🛒", "description": "Snack, dry food & more"},
    {"name": "Wraps & Liners", "icon": "🧻", "description": "Food wraps, liners & serving paper"},
    {"name": "Cutlery", "icon": "🍴", "description": "Disposable forks, spoons & sets"},
]
...
def init_db():
    ...
    if "customer_email" not in review_columns:
        conn.execute(
            "ALTER TABLE reviews ADD COLUMN customer_email TEXT NOT NULL DEFAULT ''"
        )

    if "review_photos" not in review_columns:
        conn.execute(
            "ALTER TABLE reviews ADD COLUMN review_photos TEXT"
        )
...
def checkout():
    ...
    # Notification failure must never cancel an otherwise successful order.
...
def send_new_order_notification(order_id, order):
    """Email the Packaroji team when a new order is successfully saved."""
    if not (SMTP_HOST and SMTP_USERNAME and SMTP_PASSWORD and SMTP_FROM_EMAIL and ORDER_NOTIFICATION_EMAIL):
        app.logger.warning("New-order email not sent: SMTP/order notification settings are incomplete.")
        return False

    subject = f"New Packaroji order PK-{int(order_id):05d}"
    message = EmailMessage()
    message["Subject"] = subject
    message["From"] = SMTP_FROM_EMAIL
    message["To"] = ORDER_NOTIFICATION_EMAIL

    message.set_content(
        f"""A new order has been placed on Packaroji.

Order: PK-{int(order_id):05d}
Customer: {order.get('customer_name') or '—'}
Business: {order.get('business_name') or '—'}
...
Please open the Packaroji Admin Dashboard to review the complete order and any customization files.
...
"""
    ...
    try:
        ...
        app.logger.exception("Unable to send new-order notification email")
        return False
...
def send_password_reset_email(email, name, reset_url):
    ...
    message = EmailMessage()
    message["Subject"] = "Reset your Packaroji password"
    message["From"] = SMTP_FROM_EMAIL
    message["To"] = email
...
@app.get("/uploads/reviews/<path:filename>")
def review_upload(filename):
    safe_name = secure_filename(filename)
    if safe_name != filename:
        return "Not Found", 404
    return send_from_directory(REVIEW_UPLOAD_DIR, safe_name)
...
def submit_review():
    ...
    if (
        ...
        return jsonify({
            "ok": False,
            "message": "Please provide your name, email, review and rating.",
        }), 400

    photos = []
    for file in request.files.getlist("review_photos"):
        if not file or not file.filename:
            continue
        extension = secure_filename(file.filename).rsplit(".", 1)[-1].lower() if "." in file.filename else ""
        if extension not in {"png", "jpg", "jpeg", "webp"}:
            continue
        filename = f"review-{uuid.uuid4().hex}.{extension}"
        file.save(os.path.join(REVIEW_UPLOAD_DIR, filename))
    ...
    conn.execute(
        """
        INSERT INTO reviews (
            created_at, customer_name, customer_email, rating,
            review_text, review_photos, approved
        )
        VALUES (?, ?, ?, ?, ?, ?, 0)
        """,
        (utc_now(), customer_name, customer_email, rating, review_text, json.dumps(photos))
    )
    conn.commit()
    conn.close()
