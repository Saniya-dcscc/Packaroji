...
SMTP_HOST = os.environ.get("PACKAROJI_SMTP_HOST", "smtp.gmail.com")
SMTP_PORT = int(os.environ.get("PACKAROJI_SMTP_PORT", "465"))
SMTP_USERNAME = os.environ.get("PACKAROJI_SMTP_USERNAME", BUSINESS["email"])
...
def init_db():
    ...
    if "email_otp_attempts" not in user_columns:
...
@app.get("/privacy-policy", endpoint="privacy_policy")
def privacy_policy():
    return render_template("privacy_policy.html")


@app.get("/cookie-policy", endpoint="cookie_policy")
def cookie_policy():
...
def product_collection_page(collection_slug):
    ...
    collections = {
        ...
        "main-portion-packaging": {"name":"Main Portion Packaging","description":"Sustainably sourced materials — aluminium, kraft paper, and sugarcane bagasse — engineered for heat retention, grease resistance, and full biodegradability, from the first wrap to the final bite.","kind":"layer2","image":"catalogue/layer2-main-portion.jpg","image_alt":"Sugarcane bagasse food container","products":lambda: [p for p in PRODUCTS if p.get("layer") == "Layer 2 — Main Portion Packaging"]},
        "takeout-delivery": {"name":"Takeout & Delivery","description":"Takeout & delivery packaging designed for food businesses, from everyday takeaway bags to higher-capacity delivery formats.","kind":"layer3","image":"catalogue/layer3-takeout-delivery.jpg","image_alt":"Kraft takeaway and delivery bags","products":lambda: [p for p in PRODUCTS if p.get("layer") == "Layer 3 — Takeout & Delivery"]},
        "branding": {"name":"Branding Options","description":"Branded packaging that helps cafes, bakeries, restaurants and food-service businesses carry their identity from the first impression to the final bite.","kind":"branding","image":"catalogue/layer4-branding.jpg","image_alt":"Custom branded food packaging","products":lambda: [p for p in PRODUCTS if p.get("brandable")]},
    }
...
def send_email(to_email, subject, body):
    ...
    if not to_email:
        app.logger.warning("Packaroji email not sent: recipient is missing.")
        return False

    if not _smtp_ready():
        app.logger.error(
            "Packaroji email not sent: SMTP is not configured. "
            "Set PACKAROJI_SMTP_PASSWORD (a Gmail App Password) on the deployment environment."
        )
        return False
...
def valid_customer_password(password):
    return (
        len(password) >= 6
...
def send_email_otp(email, name, otp):
    return send_email(
        email,
        "Your Packaroji verification code",
        f"""Hello {name or 'there'},

Thanks for creating a Packaroji account.

Your email verification code is:

{otp}

This code expires in {EMAIL_OTP_MINUTES} minutes. You have up to {EMAIL_OTP_MAX_ATTEMPTS} attempts.

Enter this code on the Packaroji website to verify your email address.
...
def issue_email_otp(conn, user_id, email, name):
    otp = generate_email_otp()
    otp_hash = _email_otp_hash(email, otp)
    expires_at = (
        datetime.utcnow() + timedelta(minutes=EMAIL_OTP_MINUTES)
    ...
    if not send_email_otp(email, name, otp):
        return False
    return True
...
def reset_password(token):
    ...
    if not valid_customer_password(password):
        conn.close()
        return render_template(
            "reset_password.html",
            token=token,
            error="Password must be at least 6 characters and include a capital letter, a number, and a special character."
        )
...
def verify_otp():
    ...
    user = conn.execute(
        """
        SELECT id, name, email, email_verified, email_otp_hash,
               email_otp_expires_at, email_otp_attempts
        FROM users
        WHERE email = ?
        """,
        (email,),
    ...
    if not user:
        ...
        return render_template(
            "verify_otp.html",
            email=email,
            error="We couldn't verify that email address. Please create your account again.",
        )
    ...
    if (user["email_otp_attempts"] or 0) >= EMAIL_OTP_MAX_ATTEMPTS:
        conn.close()
        return render_template(
            "verify_otp.html",
            email=email,
    ...
    try:
        expires_at = datetime.fromisoformat(
            (user["email_otp_expires_at"] or "").rstrip("Z")
        )
    ...
    if datetime.utcnow() > expires_at:
        ...
        return render_template(
            "verify_otp.html",
            email=email,
            error="That verification code has expired. Please request a new code.",
            can_resend=True,
        )

    expected_hash = _email_otp_hash(email, otp)
    if not secrets.compare_digest(expected_hash, user["email_otp_hash"] or ""):
        attempts = (user["email_otp_attempts"] or 0) + 1
        conn.execute(
        ...
        conn.commit()
        conn.close()
        remaining = max(0, EMAIL_OTP_MAX_ATTEMPTS - attempts)
        message = (
            "Too many incorrect attempts. Please request a new code."
            if remaining == 0
            else f"That code is incorrect. You have {remaining} attempt{'s' if remaining != 1 else ''} left."
        )
        return render_template(
            "verify_otp.html",
            email=email,
...
def resend_verification():
    ...
    if user and not user["email_verified"]:
        if issue_email_otp(conn, user["id"], user["email"], user["name"]):
            conn.commit()
            conn.close()
        ...
        return render_template(
            "login.html",
            error="We couldn't send a verification code right now. Please try again in a few minutes.",
            entered_email=email,
            auth_mode="login",
        )
...
def signup():
    ...
    if not issue_email_otp(conn, user_id, email, name):
...
def login():
    ...
    if not user["email_verified"]:
        ...
        return render_template(
            "login.html",
            error=(
                "Please verify your email before logging in. Enter the 6-digit "
